...
const EditarPalabra = () => {
  ...
  useEffect(() => {
    const key = localStorage.getItem('clave');
    const fetchLexema = async () => {
      try {
        // Obtener los datos del cliente
        const response = await fetch('/api/obtener-lexema', {
          method: 'POST',
          headers: {
        ...
        const dataWord = value.data.word;
        const wordType = dataWord.type == 'SALUDO' || dataWord.type == 'DESPEDIDA'|| dataWord.type == 'IDENTIFICACION' ? dataWord.type : 'otro' ;
        setFormData({
          word:dataWord.word,
          level:dataWord.qualification,
          type:wordType
        })
      } catch (error) {
  ...
  return (
    <div className="max-w-md mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
      <div className="px-6 py-4">
        <h2 className="text-xl font-bold mb-2">Editar Palabra</h2>
